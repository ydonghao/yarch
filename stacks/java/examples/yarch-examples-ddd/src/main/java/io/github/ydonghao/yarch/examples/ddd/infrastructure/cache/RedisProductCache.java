package io.github.ydonghao.yarch.examples.ddd.infrastructure.cache;

import com.fasterxml.jackson.databind.ObjectMapper;
import io.github.ydonghao.yarch.examples.ddd.domain.model.Product;
import io.github.ydonghao.yarch.examples.ddd.domain.repository.ProductCache;
...
/**
 * Cache-Aside 的 Redis 实现（redis.md「缓存一致性」）： 读 miss 由应用服务回源后回填；写路径删缓存（DEL，禁更新缓存）；TTL 带 10%
 * 抖动防雪崩（二-5）。 key 形如 yarch-examples-ddd:product:detail:{id}——首段=服务名（租户边界）。
 */
@Component
public class RedisProductCache implements ProductCache {
    ...
    public RedisProductCache(RedisTemplate<String, Object> redis) {
        this.redis = redis;
    }
